...
class CoupondiscountController extends Controller
{
    ...
    public function actionEdit(){
        ...
        if(Yii::$app->request->isPost){
             ...
             $model = Coupondiscount::findOne($post['coupon']['id']);
             ...
             $model->min_amount = $post['coupon']['min_amount'];
            $db = Yii::$app->db;
            $transaction = $db->beginTransaction();
            try {

//                if(bcdiv($post['coupon']['discount_amount'],$post['coupon']['min_amount'],3)>0.1){
...
//                }else{
    ...
    public function actionAdd(){
        ...
        if(Yii::$app->request->isPost){
            $post = Yii::$app->request->post();
            if(!$post||!$post['coupon']['title']||!$post['coupon']['min_amount']||!$post['coupon']['discount_amount']||$post['coupon']['discount_amount']<=0||$post['coupon']['min_amount']<=0){
...
//                }else{
             ...
             //   }
                ...
                $cashModel->min_amount = $post['coupon']['min_amount'];
    ...
    public function actionScope(){
        Yii::$app->response->format = 'json';
        $id = Yii::$app->request->get('id');
        $model = Coupondiscount::findOne($id);
        $city = Admin::find()->select('admin.area,admin.city')->leftJoin('admin_role', 'admin.admin_role_id=admin_role.id')->distinct()->where('admin.status=1 and admin_role.parent_id=2 and admin.admin_role_id=10')->asArray()->all();
        $rs = [];
        if($city){
...
//列表
    public function actionLists()
    {
        ...
        if (Yii::$app->session->get('username') != 'admin') {
            ...
            if (!in_array('coupondiscountadd', $aulist)) {
                $enableAdd = true;
            }
            if (!in_array('coupondiscountedit', $aulist)) {
                $enableEdit = true;
            }
            if (!in_array('coupondiscountcheck', $aulist)) {
                $enableCheck = true;
            }
        }
